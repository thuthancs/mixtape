import { addEdge, applyEdgeChanges, applyNodeChanges, Handle, Panel, Position, ReactFlow, type Connection, type Edge, type EdgeChange, type Node, type NodeChange, type NodeProps } from '@xyflow/react';
...
const PARAM_CONFIG: Record<string, {
  ...
  topic: {
    label: 'Topic',
    placeholder: 'e.g. A rock anthem about hackathon innovation'
  },
  tags: {
    label: 'Tags',
    placeholder: 'e.g. rock, electric guitar, anthem'
  },
  prompt: {
    label: 'Custom lyrics',
    placeholder: '[Verse]\\nYour lyrics here...'
...
function ParamNode({
  ...
  data,
  type
}: NodeProps<Node<ParamData>>) {
  const paramType = type as string;
  const config = PARAM_CONFIG[paramType] ?? {
    label: paramType,
    placeholder: ''
  };
  const {
  ...
  const handleChange = useCallback((newValue: ParamValue) => {
  ...
  if (isInstrumental) {
    return <div style={nodeStyle}>
        ...
        fontSize: '13px'
          ...
          <input type="checkbox" checked={boolValue} onChange={e => handleChange(e.target.checked)} aria-label={config.label} />
          {config.label}
        </label>
  ...
  return <div style={nodeStyle}>
      ...
      {isMultiline ? <textarea value={textValue} onChange={e => handleChange(e.target.value)} placeholder={config.placeholder} rows={3} style={{
      width: '100%',
      padding: '6px 8px',
      fontSize: '13px',
    ...
    }} aria-label={config.label} /> : <input value={textValue} onChange={e => handleChange(e.target.value)} placeholder={config.placeholder} style={{
      width: '100%',
      padding: '6px 8px',
      fontSize: '13px',
      border: '1px solid #ccc',
...
function SongNode({
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      const pollForResult = async (): Promise<Clip> => {
        ...
        if (c.status === 'error') throw new SunoApiError(c.metadata?.error_message as string ?? 'Generation failed');
      ...
      const msg = e instanceof SunoApiError ? e.message : e instanceof Error ? e.message : 'Generation failed';
      setCreateError(msg);
      setNodes(prev => prev.map(n => n.id === id ? {
        ...n,
        data: {
  ...
  if (d.error) {
    return <div style={{
      ...
      minWidth: '220px'
        ...
        <button onClick={handleCreate} disabled={isGenerating} style={{
        width: '100%',
        padding: '8px 12px',
        fontSize: '14px',
        fontWeight: 600,
    ...
    return <div style={{
      ...
      minWidth: '220px'
        ...
        marginBottom: '8px'
          ...
          {hasConnections ? 'Connect parameter nodes, then click Create' : 'Connect parameter nodes (Topic, Tags, etc.) and click Create'}
        </div>
        ...
        borderRadius: '6px',
        cursor: isGenerating ? 'not-allowed' : 'pointer'
      }} className="py-1.5 px-3 text-[16px] bg-black text-white border-0 rounded-md cursor-pointer">
          {isGenerating ? 'Generating...' : 'Create'}
        </button>
        <Handle type="source" position={Position.Bottom} />
      </div>;
  ...
  return <div style={{
    ...
    minWidth: '280px'
      ...
      <audio src={d.audioUrl} controls style={{
      width: '100%',
      height: '36px',
      marginBottom: '8px'
...
function FlowWithContext() {
  ...
  const addParamNode = useCallback((type: 'topic' | 'tags' | 'prompt' | 'instrumental') => {
    ...
    setEdges
      ...
      <ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} fitView>
        ...
        margin: '10px'
          ...
          <button onClick={() => addParamNode('topic')} style={{
          padding: '6px 12px',
          fontSize: '13px',
          background: '#1a192b',
          color: 'white',
          ...
          <button onClick={() => addParamNode('tags')} style={{
          padding: '6px 12px',
          fontSize: '13px',
          background: '#1a192b',
          color: 'white',
          ...
          <button onClick={() => addParamNode('prompt')} style={{
          padding: '6px 12px',
          fontSize: '13px',
          background: '#1a192b',
          color: 'white',
          ...
          <button onClick={() => addParamNode('instrumental')} style={{
          padding: '6px 12px',
          fontSize: '13px',
          background: '#1a192b',
          color: 'white',
